%%
%
classdef p53CinemaManual3D_object_cellTracker < handle
    properties
        gui_cellTracker;
        master;
    ...
    methods
        %% Constructor
        function obj = p53CinemaManual3D_object_cellTracker(master)
            obj.gui_cellTracker = p53CinemaManual3D_gui_cellTracker(master);
            obj.centroidsTracks = CentroidTimeseries3D_withAnnotations(master.obj_fileManager.maxTimepoint, 10000, master.additionalAnnotationNames);
            obj.centroidsLocalMaxima = cell(master.obj_fileManager.numZ);
            for i=1:master.obj_fileManager.numZ
                obj.centroidsLocalMaxima{i} = CentroidTimeseries3D_withAnnotations(master.obj_fileManager.maxTimepoint, 10000, {});
            end
            obj.centroidsDivisions = CentroidTimeseries3D_withAnnotations(master.obj_fileManager.maxTimepoint, 10000, {});
            obj.centroidsDeath = CentroidTimeseries3D_withAnnotations(master.obj_fileManager.maxTimepoint, 10000, {});
            obj.master = master;
        ...
        function triggerTracking(obj, altEvent)
            p53CinemaManual3D_method_cellTracker_triggerTracking(obj,altEvent);
        end
        ...
        function deleteCellTimepoints(obj, cell_id, timepoints)
            for i=timepoints
                obj.centroidsTracks.deleteCentroid(i, cell_id);
                obj.centroidsDivisions.deleteCentroid(i, cell_id);
                obj.centroidsDeath.deleteCentroid(i, cell_id);
            end
        end

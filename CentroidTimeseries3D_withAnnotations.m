classdef CentroidTimeseries3D_withAnnotations < handle
    ...
    methods
        function obj = CentroidTimeseries3D_withAnnotations(maxTimepoint, allocationSize, annotationNames)
            pointStructure.point = zeros(allocationSize, 2);
            pointStructure.value = zeros(allocationSize, 1);
            pointStructure.z = zeros(allocationSize, 1);
        ...
        function setCentroid(obj, time, cell_id, centroid, value, z)
            ...
            obj.singleCells(time).z(cell_id) = z;
        end
        ...
        function [value, valid_cells] = getZs(obj, time)
            valid_cells = find(obj.singleCells(time).point(:,1) >0);
            value = obj.singleCells(time).z(valid_cells);
        end
        ...
        function value = getZ(obj, time, cell_id)
            value = obj.singleCells(time).z(cell_id);
        end
        ...
        function deleteCentroid(obj, timepoint, cell_id)
            ...
            obj.singleCells(timepoint).z(cell_id,:) = 0;

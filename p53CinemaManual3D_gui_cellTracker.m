...
function [f] = p53CinemaManual3D_gui_cellTracker(master)
%% Create the figure
%
...
%
        ...
        if(strcmp(etx, '.txt'))
            ...
            if(sum(validFields) < 2)
                ...
                for i=1:min(max(myCentroids.timepoint), length(master.obj_imageViewer.obj_cellTracker.centroidsTracks.singleCells))
                    ...
                    if(any(strcmp(myCentroids.Properties.VariableNames, 'z')))
                        subValues = myCentroids.value(myCentroids.timepoint == i);
                        master.obj_imageViewer.obj_cellTracker.centroidsTracks.singleCells(i).z(subIndex) = subValues;
                    else
                        master.obj_imageViewer.obj_cellTracker.centroidsTracks.singleCells(i).z(subIndex) = 1;
                    end
            ...
            if(isfield(loadStruct, 'centroidsFeatures'))
                loadStruct.centroidsTracks = loadStruct.centroidsFeatures;
            end
            ...
            for t=1:min(length(loadStruct.centroidsTracks.singleCells), length(master.obj_imageViewer.obj_cellTracker.centroidsTracks.singleCells))
                ...
                master.obj_imageViewer.obj_cellTracker.centroidsTracks.singleCells(t).z = loadStruct.centroidsTracks.singleCells(t).z;
    ...
    function mergePushbutton_Callback(source, eventdata)
        ...
        tempCell = master.obj_imageViewer.obj_cellTracker.centroidsTracks.getAvailableCellId();
        master.obj_imageViewer.obj_cellTracker.replaceCellTimepoints(selectedCell, tempCell, 1:length(master.obj_imageViewer.obj_cellTracker.centroidsTracks.singleCells));
        master.obj_imageViewer.obj_cellTracker.replaceCellTimepoints(potentialMergeCell, selectedCell, currentTimepoint:length(master.obj_imageViewer.obj_cellTracker.centroidsTracks.singleCells));
        master.obj_imageViewer.obj_cellTracker.replaceCellTimepoints(tempCell, potentialMergeCell, currentTimepoint:length(master.obj_imageViewer.obj_cellTracker.centroidsTracks.singleCells));
        master.obj_imageViewer.obj_cellTracker.deleteCellData(tempCell);

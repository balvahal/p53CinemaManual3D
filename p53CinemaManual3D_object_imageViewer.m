    ...
    methods
        ...
        function obj = p53CinemaManual3D_object_imageViewer(master)
            ...
            if obj.master.obj_fileManager.preallocateMode
                ...
                obj.imageBuffer = cast(zeros(obj.image_height, obj.image_width, obj.master.obj_fileManager.numImages, obj.master.obj_fileManager.num_z), obj.buffer_resolution);
                maxPossibleValue = 2^obj.display_resolution - 1;
                
                tic
                for i=1:obj.master.obj_fileManager.numImages
                    for j=obj.master.obj_fileManager.min_z:obj.master.obj_fileManager.num_z
                        obj.master.obj_fileManager.setProgressBar(i,obj.master.obj_fileManager.numImages*obj.master.obj_fileManager.num_z,'Loading status');
                        % Load image
                        ...
                        if(get(fileManagerHandles.hcheckboxPrimaryBackground, 'Value'))
                            ...
                            obj.imageBuffer(:,:,i,j) = cast(double(OriginalImage) / normalizationFactor * maxPossibleValue, obj.buffer_resolution);
                        else
                            obj.imageBuffer(:,:,i,j) = cast(imnormalize(OriginalImage) * maxPossibleValue, obj.buffer_resolution);
                        end
                        ...
                        if(obj.master.obj_fileManager.preprocessMode)
                            ...
                            if(~strcmp(obj.master.obj_fileManager.maximaChannel, obj.master.obj_fileManager.selectedChannel))
                                referenceImageName = obj.master.obj_fileManager.getFilename(obj.master.obj_fileManager.selectedPosition, master.obj_fileManager.maximaChannel, master.obj_fileManager.currentImageTimepoints(i));
                                referenceImage = imread(fullfile(obj.master.obj_fileManager.rawdatapath, referenceImageName));
...
%             directionality_z = sign(z - previousZ);
            ...
            if(obj.master.obj_fileManager.preprocessMode && obj.obj_cellTracker.isTracking && obj.selectedCell && ~obj.obj_cellTracker.centroidsTracks.getValue(obj.currentTimepoint, obj.selectedCell))
                ...
                if(~isinf(d1) || ~isinf(d2))
                    if(d1 < d2)
                        ...
                        obj.obj_cellTracker.centroidsTracks.setCentroid(obj.currentTimepoint, obj.selectedCell, prediction2, 0, obj.currentZ);
                    end
                end
            end
        ...
        function IM = readImage(obj, frame, z)
            ...
            [targetFile, z_index] = fileManager.getFilename(fileManager.selectedPosition, obj.currentChannel, timepoint, z);

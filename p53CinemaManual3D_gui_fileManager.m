...
function [f] = p53CinemaManual3D_gui_fileManager(master)
%% Create the figure
%
...
f = figure('Visible','off','Units','characters','MenuBar','none',...
    'Renderer','OpenGL','Name','File Manager','Position',[fx fy fwidth fheight],...
...
heditMaxHeight = uicontrol('Style','edit','Units','characters',...
    ...
    'parent',f, 'callback',{@heditMaxHeight_callBack});
...
%
    ...
    function heditMaxHeight_callBack(~,~)
        ...
        maxHeight = floor(min(maxHeight, screenSize(4) * master.ppChar(2)));
        set(heditMaxHeight, 'String', maxHeight);
    end

...
function [f] = p53CinemaManual3D_gui_imageViewer(master, maxHeight)
%% Create the figure
...
hy = fheight - hheightaxes;

sliderStep = 1/((master.obj_fileManager.numZ) - 1);
if(~isinf(sliderStep))
    hsliderExploreZ = uicontrol('Style','slider','Units','characters',...
        'Min',0,'Max',1,'BackgroundColor',[255 215 0]/255,...
        'Value',0,'SliderStep',[sliderStep sliderStep],'Position',[hx hy hwidth hheightaxes],...
        'Callback',{@sliderExploreZ_Callback});
    addlistener(hsliderExploreZ,'ContinuousValueChange',@sliderExploreZ_Callback);
else
    hsliderExploreZ = [];
end
...
hwidth = hwidthaxes;
hx = 0;
hy = fheight - hheightaxes - hheight * 1.2;
...
handles.hsliderExploreZ = hsliderExploreZ;
...
%
    function fKeyPressFcn(~,keyInfo)
        switch keyInfo.Key
            case 'period'
                if(master.obj_imageViewer.shift_ON)
                    master.obj_imageViewer.nextZ;
                else
                    master.obj_imageViewer.nextFrame;
                end
            case 'comma'
                if(master.obj_imageViewer.shift_ON)
                    master.obj_imageViewer.previousZ;
                else
            ...
            case 'backspace'
                ...
                if(currentCentroid(1) > 0)
                    ...
                    master.obj_imageViewer.obj_cellTracker.centroidsDivisions.deleteCentroid(master.obj_imageViewer.currentTimepoint, master.obj_imageViewer.selectedCell);
                    master.obj_imageViewer.obj_cellTracker.centroidsDeath.deleteCentroid(master.obj_imageViewer.currentTimepoint, master.obj_imageViewer.selectedCell);
            ...
            case 'space'
                ...
                master.obj_imageViewer.setImage;
                % Edit 2020/07/03 to ease division annotations
            ...
            case 'tab'
                master.obj_imageViewer.obj_cellTracker.firstClick = 1;
            case 'shift'
                master.obj_imageViewer.shift_ON = 1;
        end
    end

    function fKeyReleaseFcn(~, keyInfo)
        switch keyInfo.Key
            case 'shift'
                master.obj_imageViewer.shift_ON = 0;
        end
    end
...
%
    ...
    function fWindowScrollWheelFcn(~,event)
        if(master.obj_imageViewer.shift_ON)
            newZ = master.obj_imageViewer.currentZ + event.VerticalScrollCount;
            newFrame = master.obj_imageViewer.currentFrame;
        else
            newFrame = master.obj_imageViewer.currentFrame + event.VerticalScrollCount;
            newZ = master.obj_imageViewer.getSelectedCellZ(newFrame);
        end
        master.obj_imageViewer.setFrame(newFrame, newZ);
    end
...
%
    function sliderExploreStack_Callback(~,~)
        ...
        master.obj_imageViewer.setFrame(targetFrame, master.obj_imageViewer.currentZ);
    end

    function sliderExploreZ_Callback(~,~)
        currentZ = get(hsliderExploreZ,'Value');
        sliderStep = get(hsliderExploreZ,'SliderStep');
        targetZ = round((currentZ / sliderStep(1)) + 1);
        master.obj_imageViewer.setFrame(master.obj_imageViewer.currentFrame, targetZ);
    end
%%
%
...
%
    function popupMarkerSize_Callback(~,~)
        ...
        set(deathEventPatch, 'MarkerSize', markerSizeMap(sizeOption, 1)*1.25);
...
%
    function popupViewerChannel_Callback(~,~)
        master.obj_imageViewer.currentChannel = get(hpopupViewerChannel, 'String');
